...
const ProjectDetails = ({ project_id }) => {
  ...
  const fetchProjectDetails = async () => {
    try {
      const response = await axios.post(
        `http://localhost:8800/api/getprojectdetails`,
        {
          project_id,
        }
      );
      ...
      if (response.data[0].title_status == 0) {
        seteditable(true);
      }
      if (response.data[0].title_status == 1) {
        seteditable(false);
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div>
      <center>
        <h2 style={{ backgroundColor: "grey" }}>Project Details</h2>
      </center>
      {showAlert && (
